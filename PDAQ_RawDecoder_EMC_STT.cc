        ...
        { // STT data processing
            ...
            { // skip the first headers, fixed size
                ...
                else if (find_hits == false)
                {
                    ...
                    // stt_channel_offsets.end() ) { // decode only registered
                    // TDCs
                    // printf("%x
                    ...
                    if (((sub[i] >> 28) & 0xf) == 0x8) { // hit marker
                        ...
                        if (channel_nr == 0) { // ref time
                            refTime = time;
                            // cout<<"check
                            // 1"<<stt_channel_offsets[tdc_id]<<endl;
                            ...
                            // printf("channel_nr %i  stt_offset %i tdcid %x
                            // old_ch
                            // %i\n",channel_nr,stt_channel_offsets[tdc_id],tdc_id,(channel_nr
                            // + stt_channel_offsets[tdc_id]));
                        ...
                        else
                        {
                            if (edge == 1) { // rising edge
                                ...
                                // h_stt_emc_pulser_timeDiff->Fill(38400 + (time
                                // - refTime) - emc_pulser_time);
                            }
                            else
                            { // falling edge

                                if (lastRise != 0) { // only in case there was a
                                                     // rising to pair
                                    ...
                                    if (doubleHit == false) {
                                        ...
                                        a->isRef = false;
                                        // printf("channel_nr %i  stt_offset %i
                                        // tdcid %x old_ch
                                        // %i\n",channel_nr,stt_channel_offsets[tdc_id],tdc_id,(channel_nr
                                        // + stt_channel_offsets[tdc_id]));

                                        // cout<<"CHECK  :
                                        // "<<a->leadTime<<"\t"<<a->trailTime<<"\t"<<a->channel<<endl;
                                    }
                                }
                            }
                        }
