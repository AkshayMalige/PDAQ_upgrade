...
int PDAQ_Cluster_Finder(char* intree, int maxEvents = 1000000000,
                        char* outtree = "PDAQ_cluster_output.root");
                       ...
                       TTree* PDAQ_tree, Stt_Track_Event* stt_event)
{
    ...
    if (track_sanity >= MAX_FT_TOTAL_LAYERS) {
        ...
        for (Int_t tq = 0; tq < vec_tracks.size(); tq++)
        {
            ...
            // 	{
            // 	  printf("TDC :%x , Layer -%d , Straw -%d
            // \n",vec_tracks[tq][ac].tdcid,
            // vec_tracks[tq][ac].layer,vec_tracks[tq][ac].straw);
            // 	}
            // 	printf("\n*********************\n");
        }
