...
int PDAQ_Stt_Calibirator(char* intree, char* outtree, int maxEvents)
{
    ...
    for (Long_t e = 0; e < nentries; e++)
    {
        ...
        { // cout<<endl<<endl;
            ...
            SttRawHit* hit = (SttRawHit*)STT->stt_raw.tdc_hits->ConstructedAt(
                i); // retrieve particular hit
            ...
            if (hit->isRef == false) {
                ...
                else if (tc->mod == 1)
                {
                    ...
                    if (cal_hit->isRef == false && cal_hit->tdcid != 0xe103) {
                        ...
                        double pit =
                            ftGeomPar->getStrawPitch(cal_hit->station - 1);
                        double rad =
                            ftGeomPar->getStrawRadius(cal_hit->station - 1);
                        double offX = ftGeomPar->getOffsetX(
                            cal_hit->station - 1, cal_hit->layer - 1,
                            cal_hit->plane);
                        double offY = ftGeomPar->getOffsetY(
                            cal_hit->station - 1, cal_hit->layer - 1,
                            cal_hit->plane);
                        double offZ = ftGeomPar->getOffsetZ(
                            cal_hit->station - 1, cal_hit->layer - 1,
                            cal_hit->plane);
